...
CREATE INDEX IF NOT EXISTS idx_delivery_zones_active ON delivery_zones (is_active) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_delivery_zones_priority ON delivery_zones (priority DESC) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_slots_zone_date ON delivery_time_slots (delivery_zone_id, slot_date);
CREATE INDEX IF NOT EXISTS idx_slots_open ON delivery_time_slots (status, slot_date) WHERE status = 'open';
...
CREATE INDEX IF NOT EXISTS idx_reservations_expires ON slot_reservations (expires_at) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_reservations_order ON slot_reservations (order_id);

-- ==========================================
-- PRODUCT CATERING ATTRIBUTES
-- ==========================================
CREATE TABLE IF NOT EXISTS product_catering_attributes (
...
CREATE INDEX IF NOT EXISTS idx_pca_category ON product_catering_attributes (category);
CREATE INDEX IF NOT EXISTS idx_pca_diet_tags ON product_catering_attributes USING GIN (diet_tags);

CREATE TABLE IF NOT EXISTS product_zone_availability (
...
CREATE INDEX IF NOT EXISTS idx_pza_zone ON product_zone_availability (delivery_zone_id) WHERE is_available = true;

-- ==========================================
-- ORDER CATERING METADATA
-- ==========================================
CREATE TABLE IF NOT EXISTS order_catering_metadata (
...
CREATE INDEX IF NOT EXISTS idx_ocm_zone ON order_catering_metadata (delivery_zone_id);
CREATE INDEX IF NOT EXISTS idx_ocm_slot ON order_catering_metadata (time_slot_id);
CREATE INDEX IF NOT EXISTS idx_ocm_subscription ON order_catering_metadata (subscription_id);
...
CREATE INDEX IF NOT EXISTS idx_sub_next_delivery ON subscriptions (next_delivery_date) WHERE status = 'active';

-- ==========================================
-- LOYALTY
-- ==========================================
CREATE TABLE IF NOT EXISTS customer_loyalty_state (
...
CREATE INDEX IF NOT EXISTS idx_referral_referrer ON referral_codes (referrer_customer_id);
...
CREATE INDEX IF NOT EXISTS idx_wh_next_retry ON webhook_deliveries (next_retry_at) WHERE status = 'failed_retry';
CREATE INDEX IF NOT EXISTS idx_wh_event ON webhook_deliveries (event_id);
...
CREATE TABLE IF NOT EXISTS incoming_webhook_log (
...
-- DIAGNOSTIC VIEWS
-- ==========================================
CREATE OR REPLACE VIEW v_active_zones AS
SELECT
...
CREATE OR REPLACE VIEW v_slots_today AS
SELECT
...
JOIN delivery_zones dz ON dz.id = ts.delivery_zone_id
